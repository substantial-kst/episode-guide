...
const resolveRequest = function(dataArr, event) {
...
const filterData = function(event, jsonDataString) {
  ...
  if (PARAMS.GENERAL_QUERY in queryString) {
    ...
    if (!!queryString[PARAMS.NUMBER]) {
      numberFiltered = filterByEpisodeNumber(
        queryString[PARAMS.NUMBER],
        filtered
      )
      if (queryString[PARAMS.UNION_TYPE] !== 'a') {
    ...
    if (!!queryString[PARAMS.GUEST_STAR]) {
      guestFiltered = filterAllItemsByGuest(
        queryString[PARAMS.GUEST_STAR],
        filtered
      )
      if (queryString[PARAMS.UNION_TYPE] !== 'a') {
    ...
    if (!!queryString[PARAMS.EPISODE]) {
      episodeFiltered = filterByEpisodeOfSeasonNumber(
        queryString[PARAMS.EPISODE],
        filtered
      )
      if (queryString[PARAMS.UNION_TYPE] !== 'a') {
    ...
    if (!!queryString[PARAMS.SEASON]) {
      seasonFiltered = filterBySeasonNumber(
        queryString[PARAMS.SEASON],
        filtered
      )
      if (queryString[PARAMS.UNION_TYPE] !== 'a') {
...
exports.handler = event => {
  return new Promise((resolve, reject) => {
    ...
    showKey =
      (event.pathParameters && event.pathParameters.programId) ||
      event.queryStringParameters[PARAMS.SHOW_KEY]
